...
const NyteStatus = (props) => {
  ...
  const host =
    props.user && props.hostData && props.hostData.id === props.user.id
      ? true
      : false;
  ...
  //Sets which buttons to render based on user conditions
    ...
    <>
      <Box sx={{ maxWidth: "90%", pt: 1 }}>
        <Stack justifyContent="center" alignItems="center" spacing={0}>
          ...
          >
            <Typography sx={{ color: "#FFFFFF" }} align="center" variant="h4">
              Hosted By
            </Typography>
            <Typography sx={font} align="center" variant="h6">
              {firstName} {lastName}
            </Typography>
            <Typography sx={font} align="center" variant="h6">
              {date}
          ...
          >
            ...
            {mode(invited, gameStatus, host) === "Host" && (
              <Host onCancelNyte={props.onCancelNyte} />
            )}
            {mode(invited, gameStatus, host) === "Confirmed" && (
              <UserConfirmed onCancel={props.onCancel} />
            )}
            {mode(invited, gameStatus, host) === "Declined" && (
              <UserDeclined onConfirm={props.onConfirm} />
            )}
            {mode(invited, gameStatus, host) === "Not Selected" && (
              <UserNotSelected
